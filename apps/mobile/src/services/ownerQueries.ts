...
export const useUpdateOwnerCourt = () => {
  ...
  return useMutation({
    mutationFn: ({ id, data }: { id: number; data: any }) =>
      ownerService.updateOwnerCourt(id, data),
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({
        queryKey: ['ownerCourtDetail', variables.id],
      });
      queryClient.invalidateQueries({ queryKey: ['ownerCourts'] });
    },
  });
...
export const useAddCourtSlot = () => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ courtId, slot }: { courtId: number; slot: any }) =>
      ownerService.addCourtSlot(courtId, slot),
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({
        queryKey: ['ownerCourtDetail', variables.courtId],
      });
    },
  });
};
...
export const useUpdateCourtSlot = () => {
  ...
  return useMutation({
    mutationFn: ({
      ...
      slotId: number;
      slot: any;
    }) => ownerService.updateCourtSlot(courtId, slotId, slot),
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({
        queryKey: ['ownerCourtDetail', variables.courtId],
      });
...
export const useDeleteCourtSlot = () => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ courtId, slotId }: { courtId: number; slotId: number }) =>
      ownerService.deleteCourtSlot(courtId, slotId),
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({
        queryKey: ['ownerCourtDetail', variables.courtId],
      });
...
export const useAddAssistant = () => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ courtId, assistant }: { courtId: number; assistant: any }) =>
      ownerService.addAssistant(courtId, assistant),
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({
        queryKey: ['ownerCourtDetail', variables.courtId],
      });
...
export const useRemoveAssistant = () => {
  ...
  return useMutation({
    mutationFn: ({
      ...
      courtId: number;
      assistantId: number;
    }) => ownerService.removeAssistant(courtId, assistantId),
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({
        queryKey: ['ownerCourtDetail', variables.courtId],
      });
